    ...
    while True:
        ...
        for detection, cls in zip(result.boxes.xyxy, result.boxes.cls):  # Each detection includes [x1, y1, x2, y2]
            ...
            if class_name.lower() == "bolt":
                ...
                text_color = (0, 255, 255)  # Yellow for nut dimensions
            else:
                color = (255, 255, 255)  # White for unknown classes
                text_color = (0, 255, 255)  # Cyan for unknown dimensions

            # Add bounding box with bold edges (thicker lines)
            cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), color, 4)  # Bold rectangle with thickness 4

            # Add size information to the frame with bold text
